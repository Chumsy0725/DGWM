...
def parse_function(*metrics, directory="", args=None, end_signal=None):
    ...
    for key, values in metrics_results.items():
        ...
        output_individually = values
    print("===")

    return output_results, output_individually
...
def main(args, end_signal):
    ...
    if args.multi_exp:
        ...
        for directory in listdir_nohidden(args.directory, sort=True):
            ...
            results, individual_results = parse_function(
            ...
            f_results.append(individual_results)
        ...
        print("Individual performance over seeds")
        f_results = np.array(f_results)
        mean = np.mean(f_results, axis=0)
        # print(np.mean(mean))
        # print(np.std(mean))

        print(f"* average {key}: {np.mean(mean):.1f}% +- {np.std(mean):.1f}%")
    else:
        parse_function(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--multi-exp", action="store_true", help="parse multiple experiments", default=True
    )
    parser.add_argument(
        "--keyword",
        default="accuracy",
